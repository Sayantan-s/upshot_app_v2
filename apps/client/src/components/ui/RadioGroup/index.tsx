...
interface IRadioContext extends Pick<Props, "onSelect"> {
...
const Root: FC<Props> = ({ onSelect, children, defaultValue, className }) => {
  ...
  const styles = twMerge("flex flex-col space-y-2", className);

  return (
    <RadioContext.Provider value={{ onSelect, value, setValue }}>
      <div className={styles} role="radiogroup">
        {children}
      </div>
...
const Option: FC<IOptionProps> = ({ value, children }) => {
  ...
  return (
    <button
      className="w-max flex items-center justify-center space-x-2"
      onClick={handleSelect}
      role="radio"
    >
      <span className="border w-8 h-8 rounded-full flex items-center justify-center">
        {value === context?.value ? (
          <svg
            width={16}
            height={16}
...
export const RadioGroup = Object.assign(Root, { Option });
