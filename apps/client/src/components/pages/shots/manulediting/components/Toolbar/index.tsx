...
export const Toolbar = () => {
  ...
  return (
    <AnimatePresence>
      {currentlyEditingShot ? (
        <motion.nav
          initial={{ y: -100, x: '-50%' }}
          animate={{ y: 0, x: '-50%' }}
          exit={{ y: -100, x: '-50%' }}
          data-active={!!currentlyEditingShot}
          className={`fixed top-0 left-1/2 data-[active="true"]:border-gray-300 w-max mx-auto mt-10 z-10 border p-2.5 flex items-center justify-center rounded-xl shadow-md shadow-slate-900/5 bg-white`}
        >
